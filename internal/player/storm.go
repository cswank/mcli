...
func (b *StormHistory) Save(r Result) error {
	...
	if err == storm.ErrNotFound {
		return b.db.Save(&StormEntry{ID: r.Track.ID, Time: time.Now().Format(time.RFC3339), Result: r})
	}
	...
	return b.db.Update(&StormEntry{ID: r.Track.ID, Time: time.Now().Format(time.RFC3339), Result: r})
}
